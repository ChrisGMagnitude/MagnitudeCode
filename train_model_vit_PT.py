...
initial_weights = r'/mnt/magbucket/dinov2-output-pretrained/eval/training_123999.pth'
...
if initial_weights != 'default':
    model.load_state_dict(torch.load(os.path.join(initial_weights), weights_only=False,map_location=torch.device(device))['model'])
    model.eval()
